...
% Last Modified by GUIDE v2.5 01-May-2025 17:14:01
...
ConductorDiamter =str2double(get(handles.conductorD,'string'));


val = get(handles.popupmenu1,'value');

GMR = 0.7788*(ConductorDiamter/2);

Area = (pi*(ConductorDiamter)^2)/4 ;
Resistance = (Resistivity * ConductorLength)/Area ;
switch val 
    case 1 
        Spacing = str2double(get(handles.spacing,'string')); 

        if isnan(Spacing) || Spacing <= 0 || isnan(GMR) || GMR <= 0 || Spacing <= GMR
             errordlg('Invalid Symmetrical Spacing or Conductor Diameter input.','Input Error');
             return; 
        end
        Inductance = ((2e-7)*log(Spacing/GMR))*ConductorLength;
        r = ConductorDiamter/2;
